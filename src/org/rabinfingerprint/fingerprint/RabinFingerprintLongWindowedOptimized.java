...
public class RabinFingerprintLongWindowedOptimized extends RabinFingerprintLong implements WindowedFingerprint<Polynomial> {
	...
	public RabinFingerprintLongWindowedOptimized(RabinFingerprintLongWindowed that, byte [] chunk, int startPos) {
		super(that);
		this.chunk = chunk;
	...
	public void pushByte(int pos) {
		...
		if(wp >=bytesPerWindow) {
			byte k = chunk[pos-bytesPerWindow];

...
public class EnhancedFingerFactory2 {
	...
	public EnhancedFingerFactory2(Polynomial p, long bytesPerWindow, ChunkBoundaryDetector boundaryDetector) {
	...
	public void getChunkFingerprints(byte[] k, EnhancedChunkVisitor visitor) throws IOException {
		...
		int sp = MIN_CHUNK_SIZE;
		if(k.length < sp)
			sp = k.length;
		RabinFingerprintLongWindowedOptimized window = new RabinFingerprintLongWindowedOptimized(fingerWindow,k,sp);
		// counters
		int chunkStart = 0;
		int chunkEnd = 0;
		int chunkLength = 0;
